...
class ASLWordPredictor:
                                ...
                                if not self.letter_buffer or new_letter != self.letter_buffer[-1]:
                                    ...
                                    if self.suggestions:
                                        ...
                                        if len(self.suggestions) == 1:
                                            ...
                                            print(f"Word locked: {self.current_word}")
                                            self.play_audio(self.current_word)  # Generate and play TTS audio
                                            #self.play_audio("Arathi")
                                            self.reset_state()
                                        else:
    ...
    def play_audio(self, word):
        try:
            ...
            client = ElevenLabs(api_key=self.tts_api_key)

            print(f"Playing audio for word: {word}")
            audio = client.generate(
                text=word,
                voice=Voice(self.voice_id)
            )
            play(audio)
        except Exception as e:
